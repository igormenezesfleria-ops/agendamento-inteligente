...
export default function TeamManagement() {
  ...
  const getPayLabel = (payType: string | null) => {
    switch (payType) {
      case 'fixed_monthly': return 'Salário Fixo';
      case 'per_student': return 'Por Aluno';
      case 'per_class': return 'Por Aula';
      default: return 'Treinador';
    }
  };
    ...
    <DashboardLayout>
      <div className="space-y-6 animate-fade-in pb-32">
        <div className="text-center space-y-1 pt-2">
          <h1 className="text-3xl font-extrabold text-foreground">Sua Equipe.</h1>
          <p className="text-muted-foreground text-sm">Gerencie os colaboradores do seu studio.</p>
        </div>
        ...
        <div className="bg-card rounded-2xl p-4 border border-border shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-2xl font-extrabold text-foreground">{collaborators?.length ?? 0}</p>
        ...
        {isLoading ? (
          ...
          <div className="space-y-3">
            {collaborators.map((collaborator) => (
              <div
                key={collaborator.id}
                className="bg-card rounded-2xl p-5 border border-border shadow-sm flex items-center gap-4 cursor-pointer hover:border-accent/30 transition-all"
                onClick={() => {
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    {getPayLabel(collaborator.pay_type)}
                  </p>
                </div>
                <div className="flex items-center gap-1 shrink-0" onClick={(e) => e.stopPropagation()}>
                  <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-accent"
                    onClick={() => { setSelectedCollab({ id: collaborator.id, name: collaborator.name }); setHistoryDialogOpen(true); }} title="Ver histórico">
                    <Eye className="w-4 h-4" />
                  </Button>
                  <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-accent"
                    onClick={() => { setRatesCollab(collaborator); setRatesDialogOpen(true); }} title="Configurar pagamento">
          ...
          <div className="bg-card rounded-2xl p-10 border border-border shadow-sm text-center">
            ...
            <h3 className="text-xl font-extrabold text-foreground mb-2">Nenhum colaborador</h3>
            <p className="text-muted-foreground text-sm mb-6">Adicione membros à sua equipe para delegar treinos.</p>
          </div>
        )}

        {/* CTA */}
        <Button variant="accent" className="w-full py-4 h-auto rounded-xl font-bold text-lg shadow-md" onClick={() => setIsAddDialogOpen(true)}>
          ...
          <DialogContent>
            ...
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                ...
                <Input id="name" placeholder="Nome do colaborador" value={newCollaborator.name}
                  onChange={(e) => setNewCollaborator({ ...newCollaborator, name: e.target.value })}
                  className="bg-muted border-border rounded-xl px-4 py-3 focus:ring-2 focus:ring-accent focus:border-accent focus:bg-card transition-all" />
              </div>
              <div className="space-y-2">
                <Label htmlFor="email">E-mail</Label>
                <Input id="email" type="email" placeholder="email@exemplo.com" value={newCollaborator.email}
                  onChange={(e) => setNewCollaborator({ ...newCollaborator, email: e.target.value })}
                  className="bg-muted border-border rounded-xl px-4 py-3 focus:ring-2 focus:ring-accent focus:border-accent focus:bg-card transition-all" />
              </div>
              <div className="space-y-2">
                <Label htmlFor="password">Senha inicial</Label>
                <Input id="password" type="password" placeholder="Mínimo 6 caracteres" value={newCollaborator.password}
        ...
        <CollaboratorHistoryDialog open={historyDialogOpen} onOpenChange={setHistoryDialogOpen}
          collaboratorId={selectedCollab?.id || null} collaboratorName={selectedCollab?.name || null} />
        <CollaboratorRatesDialog open={ratesDialogOpen} onOpenChange={setRatesDialogOpen} collaborator={ratesCollab} />
      </div>
    </DashboardLayout>
  );
}
