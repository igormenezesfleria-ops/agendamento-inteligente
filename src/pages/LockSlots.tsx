...
export default function LockSlots() {
  ...
  const addLockedSlot = async () => {
    ...
    try {
      ...
      if (existing) {
        toast({
          title: 'Horário já trancado',
          description: 'Este horário já está bloqueado.',
          variant: 'destructive',
        });
      ...
      toast({
        title: 'Horário trancado!',
        description: `${format(selectedDate, 'dd/MM/yyyy')} às ${selectedTimeSlot} foi bloqueado.`,
      });
      ...
      toast({
        ...
        description: error.message || 'Não foi possível trancar o horário.',
        variant: 'destructive',
      });
    } finally {
      setIsSubmitting(false);
  ...
  const removeLockedSlot = useMutation({
    ...
    onError: (error: any) => {
      toast({
        ...
        description: error.message || 'Não foi possível liberar o horário.',
        variant: 'destructive',
      });
    },
  });
          ...
          <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Selecione uma data e horário para bloquear agendamentos.
                </DialogDescription>
              </DialogHeader>
              ...
              <DialogFooter>
                ...
                <Button variant="accent" onClick={addLockedSlot} disabled={isSubmitting}>
                  {isSubmitting ? (
                    <>
                          ...
                          <AlertDialog>
                            ...
                            <AlertDialogContent>
                              <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                  O horário {slot.time_slot} de{' '}
                                  {format(new Date(slot.date + 'T12:00:00'), 'dd/MM/yyyy')}{' '}
                                  será liberado para agendamentos.
                                </AlertDialogDescription>
                              </AlertDialogHeader>
                              <AlertDialogFooter>
                                <AlertDialogCancel>Cancelar</AlertDialogCancel>
                                <AlertDialogAction
                                  onClick={() => removeLockedSlot.mutate(slot.id)}
                                >
                                  Liberar
                                </AlertDialogAction>
                              </AlertDialogFooter>
          ...
          <Card>
            <CardContent className="p-12 text-center">
              ...
              <h3 className="font-display text-xl text-foreground mb-2">
                Nenhum horário trancado
              </h3>
              <p className="text-muted-foreground mb-6">
                Tranque horários para impedir que alunos façam agendamentos.
              </p>
              <Button variant="accent" onClick={() => setIsAddDialogOpen(true)}>
                <Plus className="w-4 h-4 mr-2" />
                Trancar Primeiro Horário
              </Button>
            </CardContent>
          </Card>
        )}
